...
//        rightstick = ctx =>
//        {
//            float xValue = Input.GetAxis("RThumbstickHorizontal");
//            float yValue = Input.GetAxis("RThumbstickVertical");
...
public class ControladorDeCamara : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Obtener la rotación del joystick derecho
        Vector2 rightstick = OVRInput.Get(OVRInput.RawAxis2D.RThumbstick);


        // Calcular la rotación vertical (eje X) limitando el ángulo
        currentRotation.x = ClampRotation(currentRotation.x - rightstick.y * rotateSpeed * Time.deltaTime, minRotacionX, maxRotacionX);

        // Calcular la rotación horizontal (eje Y)
        currentRotation.y += rightstick.x * rotateSpeed * Time.deltaTime;
